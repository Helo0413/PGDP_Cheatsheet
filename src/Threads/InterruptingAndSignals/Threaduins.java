...
public final class Threaduins {
	...
	public static Thread getLuckyProcrastinator(PrintStream s) {
		return new Thread() {
			...
			public void run() {
				...
				synchronized (this) {
					try {
						// Let penguin procrastinate and wait until notified
						this.wait();
					} catch (InterruptedException e) {
						e.printStackTrace();
						interrupt();
	...
	public static void stopProcrastinator(Thread procrastinator) {
		...
		try {
			System.out.println(STOP_MSG);
			//waits for a signal in the console
			signal.await();
			synchronized (procrastinator) {
				//notifies the Thread! U may now go and work -> if multiple threads are operating on the same
				// procrastinator object concurrently and one thread calls notify() while another thread is
				// modifying the state of procrastinator, it could lead to unpredictable behavior or even
				// corruption of the object's state.
