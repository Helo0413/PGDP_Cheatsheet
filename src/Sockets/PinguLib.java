...
public class PinguLib {
	...
	/**
	 * Reads a {@link String} from the console, and prompts the user by printing
	 * the given <code>text</code> with a line break to the console.
	 * <p>
	 *
	 * <i>Liest einen {@link String} von der Konsole ein, und fordert den oder
	 * die Benutzerin durch die Ausgabe des übergebenen <code>text</code>es zur
	 * Eingabe auf (mit Zeilenumbruch).</i>
	 *
	 ...
	 *
	 * <i>Liest einen {@link String} von der Konsole ein, und fordert den oder
	 * die Benutzerin dazu auf durch die Ausgabe von
	 *
	 * <pre>
	 * Eingabe:
	 * </pre>
	...
	/**
	 * Tries to read an <code>int</code> from the console, and retries if the
	 * input was not a valid integer. It prompts the user by printing the given
	 * <code>text</code> with a line break to the console.
	 * <p>
	 * <i>Versucht einen <code>int</code>-Wert von der Konsole einzulesen, und
	 * wiederholt die Anfrage solange es nicht nicht um eine ganze Zahl handelt.
	 * Der oder die Benutzerin wird durch die Ausgabe des übergebenen
	...
	/**
	 * Works like {@link #readInt(String)}, but uses the following preset text
	 * to prompt the user:
	 *
	 * <pre>
	...
	/**
	 * Tries to read a single <code>char</code> from the console, and retries if
	 * the input was not a single character. It prompts the user by printing the
	 * given <code>text</code> with a line break to the console.
	 * <p>
	 * <i>Versucht einen <code>char</code>-Wert von der Konsole einzulesen, und
	 * wiederholt die Anfrage solange es nicht um ein einzelnes Zeichen handelt.
	 * Der oder die Benutzerin wird durch die Ausgabe des übergebenen
	 * <code>text</code>es zur Eingabe aufgefordert (mit Zeilenumbruch).</i>
	 */
	...
	/**
	 * Works like {@link #readChar(String)}, but uses the following preset text
	 * to prompt the user:
	 *
	 * <pre>
	...
	/**
	 * Tries to read an <code>double</code> from the console, and retries if the
	 * input was not a valid double value. It prompts the user by printing the
	 * given <code>text</code> with a line break to the console.
	 * <p>
	 * <i>Versucht einen <code>double</code>-Wert von der Konsole einzulesen,
	 * und wiederholt die Anfrage solange es nicht nicht um eine gültige
	...
	/**
	 * Works like {@link #readDouble(String)}, but uses the following preset
	 ...
	 * <p>
	 * <i>Gibt den <code>int</code>-Wert auf der Konsole aus und beginnt eine
	 * neue Zeile.</i>
	 */
	...
	/**
	 * Prints the <code>int</code> to the console. Does not end with a line
	 * break.
	 * <p>
	 * <i>Gibt den <code>int</code>-Wert auf der Konsole aus. Endet nicht mit
	 ...
	 * <p>
	 * <i>Zieht eine zufällige Karte, repräsentiert durch ihren Wert. Der Wert
	 * liegt im Bereich 1 (inklusive) bis 105 (inklusive).</i>
	 */
