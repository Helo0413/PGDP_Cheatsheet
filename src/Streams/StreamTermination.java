...
public class StreamTermination {
    ...
    public static Fish[] sellFishArray(Stream<Fish> stream) {
       // Fish[]:: new creates a new array of type fish;
        // :: substitutes the parameter. so instead of (size) -> new Fish[size]
        // you go Fish[]::new
    ...
    public static void consumeTrouts(Stream<Fish> stream) {
        stream.filter(x ->  x.getSpecies() == Fish.FishSpecies.TROUT)
                .forEach(x -> System.out.println("Sofia fand TROUT mit Gewicht " + x.getWeight() + "kg sehr lecker!"));
    }
